...
__global__ void matrixMulTile(float *m1, float *m2, float *m3, int N);

/* naive matrix multiplication by GPU using global memory */
__global__ void matrixMulTile(float *m1, float *m2, float *m3, int N){
    
    // create shared submatrix
    __shared__ float subM1[TILE_WIDTH][TILE_WIDTH];
    __shared__ float subM2[TILE_WIDTH][TILE_WIDTH];

    // identify tile and thread x & y
    int bx = blockIdx.x;
    int by = blockIdx.y;
    int tx = threadIdx.x;
    int ty = threadIdx.y;

    // each thread compute one element of m3
    int row = by * TILE_WIDTH + ty;
    int col = bx * TILE_WIDTH + tx;

    float pSum = 0;
    // loop through all tiles of the corresponding row and column
    for (int i = 0; i < ceil(N/(float)TILE_WIDTH); i++){
        
        // each thread load one element of m1 and m2 to shared memory tile
        if ( (row < N) && (i*TILE_WIDTH) < N) // check boundary condition
        {
            subM1[ty][tx] = m1[row * N + i * TILE_WIDTH + tx];
        }

        if ((i*TILE_WIDTH+ty) < N && col < N)
        {
            subM2[ty][tx] = m2[( i * TILE_WIDTH + ty) * N + col];
        }

        __syncthreads(); //make sure all threads finish loading before computing partial sum

        for (int j = 0; j < TILE_WIDTH; j++){
            pSum += subM1[ty][j] * subM2[j][tx];
        }

        __syncthreads(); //make sure all threads finish computing partial sum before assigning value

        if( (row < N) && (col < N) ) m3[row * N + col] = pSum;
    }
}
...
int main(int argc, char *argv[]){
    ...
    matrixMulTile<<<dimGrid, dimBlock>>>(dev_a, dev_b, dev_c, width);
